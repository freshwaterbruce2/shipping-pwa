    ...
    >
      {isProcessing && !interimTranscript ? (
        <div className="flex items-center">
          <Volume className={`${isMobile ? "h-4 w-4" : "h-3 w-3"} text-blue-600 mr-2 animate-pulse`} />
          <span className="text-blue-700 font-semibold">🎤 Listening...</span>
          <span className="ml-2 inline-block animate-pulse text-blue-500">●●●</span>
        </div>
      ) : interimTranscript ? (
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            {isFinal && <Check className={`${isMobile ? "h-4 w-4" : "h-3 w-3"} text-green-600`} />}
          </div>
          {recentCommand && (
            <div className="flex items-center gap-2 mt-2 p-2 bg-blue-50 rounded">
              <span className="font-bold text-blue-700">Command:</span>
              <span className={`font-semibold ${getConfidenceColor()}`}>{recentCommand}</span>
            </div>
          )}
        </div>
      ) : (
        <div className="flex flex-col space-y-1">
          ...
          <span className={`text-blue-500 font-medium ${isMobile ? "text-xs" : "text-[10px]"}`}>
