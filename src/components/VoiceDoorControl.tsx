...
const VoiceDoorControl = ({ onAddDoor, onAddDoorWithParams }: VoiceDoorControlProps) => {
  ...
  const doorCommandPatterns = [
    {
      ...
      regex: /(?:door|dock|bay|trailer|spot)\s+(\d{3})\s+(?:to|for|going\s+to|destination)\s+(?:dc\s+)?(\d{4})\s+(?:with|type|freight\s+type)?\s*(xd|cross\s*dock|28|twenty\s*eight|23\s*\/?\s*43|aib|air)?\s*(?:status\s+)?(empty|partial|shipload|25\s*%?|50\s*%?|75\s*%?)?/i,
      commandName: "add complete door",
      feedback: "Adding complete door setup"
    },
    {
      // SPEED: Door + DC only (most common)
      regex: /(?:door|dock|bay|trailer|spot)\s+(\d{3})\s+(?:to|for|going\s+to|destination)\s+(?:dc\s+)?(\d{4})/i,
      commandName: "add door with dc",
      feedback: "Adding door with DC",
    },
    {
      // SPEED: Door + Freight Type
      regex: /(?:door|dock|bay|trailer|spot)\s+(\d{3})\s+(?:with|type|freight\s+type)\s+(xd|cross\s*dock|cross\s*docking|28|twenty\s*eight|23\s*\/?\s*43|twenty\s*three\s*forty\s*three|aib|air|express|priority|standard)/i,
      commandName: "add door with freight",
      feedback: "Adding door with freight type",
    },
    {
      // SPEED: Just door number (fastest)
      regex: /(?:door|dock|bay|trailer|spot)\s+(?:number\s+)?(\d{3})|(?:add|new|create|setup)\s+(?:door|dock|bay|trailer|spot)\s+(\d{3})|(\d{3})\s+(?:door|dock|bay|trailer|spot)/i,
      commandName: "add specific door",
      feedback: "Adding door number",
    },
    {
      // Basic commands - multiple variations with warehouse terminology
      regex: /(?:add|new|create|start|open|begin|setup|make)\s+(?:door|dock|bay|trailer|load|spot)|(?:door|dock|bay|trailer|load|spot)(?:\s+please)?$/i,
    ...
    },
    {
      ...
      regex: /(empty|partial|shipload|full|loaded|twenty\s*five\s*percent|fifty\s*percent|seventy\s*five\s*percent|25\s*%?|50\s*%?|75\s*%?|quarter|half|three\s*quarters)\s+(?:door|dock|bay|trailer|load|spot)/i,
    ...
    },
    {
      ...
      regex: /(?:quick|fast|rush|urgent|priority|asap|immediate|now)\s+(?:door|dock|bay|trailer|spot)|(?:door|dock|bay|trailer|spot)\s+(?:quick|fast|rush|urgent|priority|asap|now)/i,
      commandName: "quick add door",
      feedback: "Adding priority door",
    },
    {
      // TCR commands - trailer condition report with variations
      regex: /(?:tcr|trailer\s+condition\s+report|condition\s+report|paperwork)\s+(present|available|yes|here|good|missing|not\s+available|no|none|absent)/i,
      commandName: "tcr status",
      feedback: "Updating TCR status",
    },
    {
      // Pallet count commands with warehouse terminology
      regex: /(?:door|dock|bay|trailer|spot)\s+(\d{3})\s+(?:has|contains|loaded\s+with|count|carrying)\s+(\d+)\s+(?:pallets?|units?|pieces?|skids?)/i,
      commandName: "set pallet count",
      feedback: "Setting pallet count",
    },
    {
      // Update existing door commands with warehouse language
      regex: /(?:update|change|modify|set|switch|move)\s+(?:door|dock|bay|trailer|spot)\s+(\d{3})\s+(?:to|for|destination)\s+(?:dc\s+)?(\d{4})/i,
      commandName: "update door destination",
      feedback: "Updating door destination",
    },
    {
      // Remove door commands with warehouse terminology
      regex: /(?:remove|delete|cancel|close|clear|finish|done\s+with)\s+(?:door|dock|bay|trailer|spot)\s+(\d{3})/i,
      commandName: "remove door",
      feedback: "Removing door",
    },
    {
      // Natural language variations with warehouse context
      regex: /(?:i\s+need|please\s+add|can\s+you\s+add|set\s+up|create|make\s+me)\s+(?:a\s+)?(?:door|dock|bay|trailer|spot|new\s+entry)/i,
      commandName: "add door",
      feedback: "Adding new door",
    },
    {
      // Emergency/special handling commands
      regex: /(?:emergency|special|hot|expedite|rush\s+order)\s+(?:door|dock|bay|trailer|spot)/i,
  ...
  const {
    ...
    onCommandRecognized: (commandName, paramsArray) => {
        ...
        if (commandName === 'add door' || commandName === 'quick add door' || commandName === 'emergency door') {
             ...
             switch (commandName) {
                 ...
                 case 'add complete door':
                     ...
                     if (paramsArray[3]) {
                         ...
                         else if (status.includes('partial')) params.trailerStatus = 'partial';
                         else if (status.includes('shipload') || status.includes('full')) params.trailerStatus = 'shipload';
                 ...
                 case 'add dc door':
                     ...
                     if (dcNum && destinationDCOptions.includes(dcNum as DestinationDC)) {
                 ...
                 case 'add status door': {
                     ...
                     const status = paramsArray[0]?.toLowerCase().replace(/\s+/g, ' ').trim();
                 ...
                 case 'add freight door': {
                     ...
                     const freight = paramsArray[0]?.toLowerCase().replace(/\s+/g, ' ').trim();
                     ...
                     const mappedFreight = freightMappings[freight];
                     if (mappedFreight) {
                  ...
                  case 'tcr status': {
                     ...
                     if (tcrStatus && ['present', 'available', 'yes'].includes(tcrStatus)) {
                         params.tcrPresent = true;
                     } else if (tcrStatus && ['not available', 'missing', 'no'].includes(tcrStatus)) {
                 ...
                 case 'set pallet count': {
                     ...
                     const palletCount = parseInt(paramsArray[1]);
                 ...
                 case 'update door destination': {
                     ...
                     if (doorNumber && dcNumber && destinationDCOptions.includes(dcNumber as DestinationDC)) {
                         params.doorNumber = doorNumber;
                 ...
                 case 'set trailer status':
                     if (paramsArray[0] && validateTrailerStatus(paramsArray[0].toLowerCase())) {
                         params.trailerStatus = paramsArray[0].toLowerCase() as TrailerStatus;
                     }
                     break;
                 default:
        ...
        if (Object.keys(params).length > 0) {
            onAddDoorWithParams(params);
             ...
             console.warn(`Command ${commandName} recognized but no parameters parsed. Falling back.`);
  ...
  const toggleHandsFreeMode = () => {
    ...
    if (newMode) {
      ...
      toast.success("Hands-free voice mode activated", {
        description: "Voice recognition will stay active. Say 'Add Door' to begin.",
        duration: 3000,
      });
    } else if (isListening) {
      stopListening();
  ...
  ].filter((option): option is { value: TrailerStatus; label: string } =>
    ...
    <div className="relative">
      <div className="flex items-center gap-2">
        <VoiceCommandButton
          ...
          label={settings.voiceActivationMode === "hotword" ? 'Say "Add Door"' : "Voice Command"}
