...
export class ShippingService {
  ...
  static isValidDoorNumber(doorNumber: number): boolean {
    return doorNumber >= this.VALID_DOOR_RANGE.min && doorNumber <= this.VALID_DOOR_RANGE.max;
  }

  /**
   * Validates a complete door schedule object
   */
  static validateDoorSchedule(door: Partial<DoorSchedule>): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];

    if (!door.doorNumber || !this.isValidDoorNumber(door.doorNumber)) {
      errors.push(`Door number must be between ${this.VALID_DOOR_RANGE.min} and ${this.VALID_DOOR_RANGE.max}`);
    }
  ...
  static async saveDoorSchedules(doors: DoorSchedule[]): Promise<void> {
    try {
      ...
      const validDoors = doors.filter(door => this.validateDoorSchedule(door).isValid);
      
      if (validDoors.length !== doors.length) {
        console.warn(`${doors.length - validDoors.length} invalid doors were filtered out during save`);
      }

      localStorage.setItem(this.STORAGE_KEY, JSON.stringify(validDoors));
    } catch (error) {
  ...
  static async addDoorSchedule(door: DoorSchedule): Promise<DoorSchedule[]> {
    ...
    const existingDoor = existingDoors.find(d => d.doorNumber === door.doorNumber);
    if (existingDoor) {
   ...
   */
  static async updateDoorSchedule(doorId: string, updates: Partial<DoorSchedule>): Promise<DoorSchedule[]> {
    const existingDoors = await this.loadDoorSchedules();
    ...
    if (!validation.isValid) {
      throw new Error(`Invalid door schedule update: ${validation.errors.join(', ')}`);
    }

    // Check for duplicate door numbers (excluding current door)
    const duplicateDoor = existingDoors.find(
      (d, index) => index !== doorIndex && d.doorNumber === updatedDoor.doorNumber
    );
    if (duplicateDoor) {
   ...
   */
  static async getDoorByNumber(doorNumber: number): Promise<DoorSchedule | null> {
   ...
   */
  static async updatePalletCount(doorId: string, newCount: number): Promise<DoorSchedule[]> {
    if (newCount < 0) {
   ...
   */
  static async incrementPalletCount(doorId: string, increment: number = 1): Promise<DoorSchedule[]> {
