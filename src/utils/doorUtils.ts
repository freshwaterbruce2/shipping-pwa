 ...
 */
export const findNextAvailableDoorNumber = (currentDoors: DoorSchedule[]): number => {
 ...
 */
export const findNextSequentialDoorNumber = (currentDoors: DoorSchedule[]): number => {
  // If no doors exist, start with 332
  if (currentDoors.length === 0) {
    return 332;
  }
  ...
  const sortedDoors = [...currentDoors].sort((a, b) => a.doorNumber - b.doorNumber);
 ...
 */
export const isDoorNumberInUse = (doorNumber: number, currentDoors: DoorSchedule[]): boolean => {
